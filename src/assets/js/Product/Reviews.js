if (window.innerWidth - 83 <= 420) {
  document.addEventListener('DOMContentLoaded', () => {
    const accArr = document.querySelectorAll('.info__tab-pages li');

    const panelArr = document.querySelectorAll('.product__info .panel');
    const contentArr = document.querySelectorAll('.product__page');

    accArr.forEach((listItem, i) => {
      if (
      ...
      let btnArrowDown = listItem.querySelector('.fa-chevron-down');
      listItem.addEventListener('click', function () {
        if (i === accArr.length - 1) {
        }
        panelArr[i].innerHTML = contentArr[i].innerHTML;

        if (panelArr[i].style.display === 'block') {
          panelArr[i].style.display = 'none';
        } else {
          panelArr[i].style.display = 'block';
        }
        ...
        if (
          (pageName =
            document.title.toLowerCase().replace(/\s/g, '') === 'product')
        ) {
          ...
          document
            ...
            .addEventListener('submit', function (event) {
              ...
              const ReviewUserName =
                document.querySelector('#ReviewUserName').value;
              const ReviewUserMail =
                document.querySelector('#ReviewUserMail').value;
