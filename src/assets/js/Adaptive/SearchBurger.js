export const searchGlobalItems = () => {
  ...
  const backgroundBlockInput = document.querySelector(
  ...
  class Items {
    ...
    createContainer() {
      ...
      container.innerHTML = `
         ...
         <p id='priceItem' class='${
           priceDiscount !== undefined ? 'discounted' : 'regular'
         }'>$ ${this.price}</p>
      ...
      container.addEventListener('click', () => {
        ...
        localStorage.setItem(
         ...
         <a href="#"id='priceItem' class="${
           priceDiscount !== undefined ? 'discounted' : 'regular'
         }">$ ${this.price}</a>
         <a href="#" class="discount">${priceDiscount ? '$ ' : ''}${
            priceDiscount || ''
          }</a></h4>`
        );
    ...
    setupEventListeners(container) {
      ...
      globalSearchInput.addEventListener('input', function () {
        ...
        itemsContainers.forEach((container) => {
          const thisName = container
            .querySelector('h1')
            .textContent.toLowerCase();
          const thisTypeProduct = container
            .querySelector('h2')
            .textContent.toLowerCase();
          const thisProductVariant = container
          ...
          if (
            thisName.includes(val) ||
            thisTypeProduct.includes(val) ||
            thisProductVariant.includes(val)
          ) {
