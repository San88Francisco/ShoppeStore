if (window.location.pathname.includes('/account')) {
   document.addEventListener("DOMContentLoaded", function () {
      ...
      function switchTabs(activeTab, inactiveTab, activeBlock, inactiveBlock) {
         ...
         }, 0.5); // Застосувати зміни через 0.1 секунди (після з'явлення блоку)
      ...
      class User {
         constructor(nameInput, lastNameInput, emailInput, passwordInput, repeatPasswordInput, selectHeaderText) {
      ...
      form.addEventListener('submit', (event) => {
         ...
         const nameInput = form.querySelector('input[name="inputName"]').value.trim();
         const lastNameInput = form.querySelector('input[name="inputLastName"]').value.trim();
         const emailInput = form.querySelector('input[name="EmailAccount"]').value.trim();
         const passwordInput = form.querySelector('input[name="inputPassword"]').value.trim();
         const repeatPasswordInput = form.querySelector('input[name="inputPasswordRepeat"]').value.trim();
         const selectHeaderText = document.querySelector('.select-header').textContent.trim();

         const user = new User(nameInput, lastNameInput, emailInput, passwordInput, repeatPasswordInput, selectHeaderText);
